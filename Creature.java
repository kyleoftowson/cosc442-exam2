...
public class Creature {
	private CreatureCreatur creatureCreatur = new CreatureCreatur();

	private CreatureEffect creatureEffect = new CreatureEffect();
	...
	public void setCreatureAi(CreatureAi ai) { creatureCreatur.setAi(ai); }
	...
	public void modifyXp(int amount) { 
		...
		while (xp > (int)(Math.pow(level, 1.75) * 25)) {
			...
			creatureCreatur.getAi().onGainLevel();
	...
	public List<Effect> effects(){ return creatureEffect.getEffects(); }
	...
	public Creature(World world, char glyph, Color color, String name, int maxHp, int attack, int defense){
		...
		creatureEffect.setEffects(new ArrayList<Effect>());
		this.maxMana = 5;
		this.mana = maxMana;
	...
	public void moveBy(int mx, int my, int mz){
		...
		Creature other = MoveByOther(mx, my, mz);
		if (other == null)
			creatureCreatur.getAi().onEnter(x+mx, y+my, z+mz, tile);
	}
	private Creature MoveByOther(int mx, int my, int mz) {
	...
	private void commonAttack(Creature other, int attack, String action, Object ... params) {
		...
		params2 = CommonAttack(params, amount, params2);
	...
	private Object[] CommonAttack(Object[] params, int amount, Object[] params2) {
	...
	private void LeaveTheCorpse() {
		for (Item item : inventory.getItems()) {
			LeavingTheCorpse(item);
		}
	}
	private void LeavingTheCorpse(Item item) {
		if (item != null)
			drop(item);
	}
	...
	public void update(){
		...
		regenerateHealth();
		regenerateMana();
		creatureEffect.updateEffects(this);
		creatureCreatur.getAi().onUpdate();
	}
	...
	public boolean canEnter(int wx, int wy, int wz) {
		return world.canEnter(wx, wy, wz);
	}

	public void notify(String message, Object ... params){
		creatureCreatur.getAi().onNotify(String.format(message, params));
	}
	...
	public boolean canSee(int wx, int wy, int wz){
		return creatureCreatur.canSee(wx, wy, wz, this.world);
	}
	...
	public Tile tile(int wx, int wy, int wz) {
		return creatureCreatur.tile(wx, wy, wz, this.world);
	}

	public Creature creature(int wx, int wy, int wz) {
		return creatureCreatur.creature(wx, wy, wz, this.world);
	}
	...
	private void DropTheItem(Item item) {
		if (world.addAtEmptySpace(item, x, y, z)) {
	...
	private void consume(Item item){
		...
		creatureEffect.addEffect(item.quaffEffect(), this);
	...
	private void addEffect(Effect effect){
		creatureEffect.addEffect(effect, this);
	}
	...
	public Item item(int wx, int wy, int wz) {
		return creatureCreatur.item(wx, wy, wz, this.world);
	}
	...
	public void throwItem(Item item, int wx, int wy, int wz) {
		...
		Creature c = creatureCreatur.creature(wx, wy, wz, this.world);
	...
	private Point Throw(int wx, int wy, Point end) {
	...
	public void modifyDetectCreatures(int amount) { creatureCreatur.setDetectCreatures(creatureCreatur.getDetectCreatures() + amount); }
	
	public void castSpell(Spell spell, int x2, int y2) {
		Creature other = creatureCreatur.creature(x2, y2, z, this.world);
	...
	public String nameOf(Item item){
		return creatureCreatur.getAi().getName(item);
	}
	
	public void learnName(Item item){
		notify("The " + item.appearance() + " is a " + item.name() + "!");
		creatureCreatur.getAi().setName(item, item.name());
	}
}
