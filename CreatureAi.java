...
public class CreatureAi {
	...
	private void Enter(int x, int y, int z, Tile tile) {
		if (tile.isGround()) {
			OnEnter(x, y, z);
		} else {
		}
	...
	private void OnEnter(int x, int y, int z) {
		creature.x = x;
		creature.y = y;
		creature.z = z;
	}
	...
	protected void useBetterEquipment() {
		...
		BetterEquipment(currentWeaponRating, currentArmorRating);
	}

	private void BetterEquipment(int currentWeaponRating, int currentArmorRating) {
		for (Item item : creature.inventory().getItems()) {
			if (item == null)
				continue;
			item.Equipment(currentWeaponRating, currentArmorRating, creature);
		}
	}
}
